...
const Home: NextPage = ({ content, postEntry }: any) => {
  const tags = postEntry?.fields?.tags?.length ? postEntry?.fields?.tags.map((tag: any) => <span className='whitespace-nowrap rounded-lg bg-slate-100 py-1 px-2 mr-2 text-xs font-semibold leading-6 text-slate-700' key={tag.fields.slug}>{tag.fields.name}</span>) : ''
    ...
    <article>
      <h1>
        ...
        <div className="my-4 flex items-center text-sm text-gray-500">
          ...
          { dayjs(postEntry.sys.createdAt).format('DD/MM/YYYY') } - wangpeng
        </div>
      </h1>
      <div>
